...
#========================================================================================
#description     : Tres osciladores acoplados con dos resortes.
...
particula3 = sphere(pos=(refX2+x2,0,0), radius=2, color=color.green)
resorte1 = helix(pos=(refX1+x1, 0, 0), axis=(refX2+x2-x1-refX1,0,0), color=(128,128,128), radius=1, coils = 25, thickness=0.4)
resorte2 = helix(pos=(refX2+x2, 0, 0), axis=(refX3+x3-x2-refX2,0,0), color=(128,128,128), radius=1, coils = 25, thickness=0.4)
...
while True:
	...
	if m1 == 0 or m2 == 0:
		...
		vx1 += p.deltat*((k/m1)*(x2-x1))
		vx2 += p.deltat*(-(k/m2)*(2*x2-x1-x3))
		vx3 += p.deltat*(-(k/m1)*(x3-x2))
		x1 += p.deltat*vx1
		x2 += p.deltat*vx2
		x3 += p.deltat*vx3
	...
	resorte1.pos = vector(refX1+x1, 0, 0)
	resorte1.axis = vector(refX2+x2-x1-refX1,0,0)
	resorte2.pos = vector(refX2+x2, 0, 0)
	resorte2.axis = vector(refX3+x3-x2-refX2,0,0)
