...
while True:
	...
	PTierra.vx -= p.deltaT*((G*(PTierra.x-PSol.x)*PSol.m)/(DistTierraSol)**(3/2))
	PTierra.vy -= p.deltaT*((G*(PTierra.y-PSol.y)*PSol.m)/(DistTierraSol)**(3/2))
	PTierra.vz -= p.deltaT*((G*(PTierra.z-PSol.z)*PSol.m)/(DistTierraSol)**(3/2))
	
	# Interaccion con el satelite
	PTierra.vx -= p.deltaT*((G*(PTierra.x-PSat.x)*PSat.m)/(DistTierraSat)**(3/2))
	PTierra.vy -= p.deltaT*((G*(PTierra.y-PSat.y)*PSat.m)/(DistTierraSat)**(3/2))
	PTierra.vz -= p.deltaT*((G*(PTierra.z-PSat.z)*PSat.m)/(DistTierraSat)**(3/2))
	
	
	### Mueve el sol ###

	# Interaccion con el satelite
	PSol.vx += p.deltaT*((G*(PSat.x-PSol.x)*PSat.m)/(DistSolSat)**(3/2))
	PSol.vy += p.deltaT*((G*(PSat.y-PSol.y)*PSat.m)/(DistSolSat)**(3/2))
	PSol.vz += p.deltaT*((G*(PSat.z-PSol.z)*PSat.m)/(DistSolSat)**(3/2))
	
	# Iteraccion con la tierra
	PSol.vx += p.deltaT*((G*(PTierra.x-PSol.x)*PTierra.m)/(DistTierraSol)**(3/2))
	PSol.vy += p.deltaT*((G*(PTierra.y-PSol.y)*PTierra.m)/(DistTierraSol)**(3/2))
	PSol.vz += p.deltaT*((G*(PTierra.z-PSol.z)*PTierra.m)/(DistTierraSol)**(3/2))
	
	
	### Mueve el satelite ###
	# Interaccion con la tierra
	PSat.vx += p.deltaT*((G*(PTierra.x-PSat.x)*PTierra.m)/(DistTierraSat)**(3/2))
	PSat.vy += p.deltaT*((G*(PTierra.y-PSat.y)*PTierra.m)/(DistTierraSat)**(3/2))
	PSat.vz += p.deltaT*((G*(PTierra.z-PSat.z)*PTierra.m)/(DistTierraSat)**(3/2))
	
	# Interaccion con el sol
	PSat.vx -= p.deltaT*((G*(PSat.x-PSol.x)*PSol.m)/(DistSolSat)**(3/2))
	PSat.vy -= p.deltaT*((G*(PSat.y-PSol.y)*PSol.m)/(DistSolSat)**(3/2))
	PSat.vz -= p.deltaT*((G*(PSat.z-PSol.z)*PSol.m)/(DistSolSat)**(3/2))
	
	PSol.x += p.deltaT*PSol.vx
	PSol.y += p.deltaT*PSol.vy
	PSol.z += p.deltaT*PSol.vz
	
	PTierra.x += p.deltaT*PTierra.vx
	PTierra.y += p.deltaT*PTierra.vy
	PTierra.z += p.deltaT*PTierra.vz
	
	PSat.x += p.deltaT*PSat.vx
	PSat.y += p.deltaT*PSat.vy
	PSat.z += p.deltaT*PSat.vz
