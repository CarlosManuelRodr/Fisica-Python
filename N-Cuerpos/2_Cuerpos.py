...
while True:
	...
	PTierra.vx -= p.deltaT*((G*(PTierra.x-PSol.x)*PSol.m)/(Dist)**(3/2))
	PTierra.vy -= p.deltaT*((G*(PTierra.y-PSol.y)*PSol.m)/(Dist)**(3/2))
	PTierra.vz -= p.deltaT*((G*(PTierra.z-PSol.z)*PSol.m)/(Dist)**(3/2))
	PTierra.x += p.deltaT*PTierra.vx
	PTierra.y += p.deltaT*PTierra.vy
	PTierra.z += p.deltaT*PTierra.vz
	
	# Mueve el sol
	PSol.vx -= p.deltaT*((G*(-PTierra.x+PSol.x)*PTierra.m)/(Dist)**(3/2))
	PSol.vy -= p.deltaT*((G*(-PTierra.y+PSol.y)*PTierra.m)/(Dist)**(3/2))
	PSol.vx -= p.deltaT*((G*(-PTierra.z+PSol.z)*PTierra.m)/(Dist)**(3/2))
